...
export default class ProductListScreen extends React.Component { 
    ...
    renderItem=({item})=>{
        return(
            <TouchableOpacity style={{flex:1,flexDirection:'row',marginBottom:3}} onPress={()=>{
                this.props.navigation.navigate('GoodListScreen',item);
            }}>
                <View>
                    <Text style={{fontSize:14,color:'green',marginBottom:5,marginTop:5}}> {item.GoodsName}</Text>
                    <Text style={{fontSize:12,marginBottom:10}}> サイス：{item.Size}</Text>
                </View>
            </TouchableOpacity>
        )     
    }
    ...
    componentDidMount() {
        ...
        fetch('http://3.115.79.185/morien/api/Goods', {
          method: 'POST',
          headers: {
        ...
        .then((responseJson)=>{
            this.setState({
                isLoading:false,
                dataSource:responseJson.goods,
                filterGoods:responseJson.goods
    ...
    onChangeText(text){
        ...
        let filterArray = this.state.filterGoods
        let searchResult = filterArray.filter(goods => 
            goods.GoodsName.includes(text)
        )
